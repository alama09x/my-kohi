    ...
    i32 height)
{
    ...
    // Show the window
    const b32 should_activate = 1 // TODO: if the window should not accept input, this should be false
...
void platform_console_write(const char *message, u8 color)
{
    ...
    const LPDWORD number_written = 0;
...
LRESULT CALLBACK win32_process_message(HWND hwnd, u32 msg, WPARAM w_param, LPARAM l_param)
{
    switch (msg) {
        ...
        case WM_MOUSEMOVE: {
            // Mouse move
            const i32 x_position = GET_X_LPARAM(l_param);
            const i32 y_position = GET_Y_LPARAM(l_param);

            // Pass to the input subsystem.
            input_process_mouse_move(x_position, y_position);
        } break;
        case WM_MOUSEWHEEL: {
            i32 z_delta = GET_WHEEL_DATA_WPARAM(w_param);
            if (z_delta != 0) {
        ...
        case WM_MBUTTONUP:
        case WM_RBUTTONUP: {
            const b8 pressed = msg == WM_LBUTTONDOWN || msg == WM_RBUTTONDOWN || msg == WM_MBUTTONDOWN;
            const buttons mouse_button = BUTTON_MAX_BUTTONS;
            switch (msg) {
                case WM_LBUTTONDOWN:
                case WM_LBUTTONUP:
