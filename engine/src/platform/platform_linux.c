    ...
    while ((event = xcb_poll_for_event(state->connection)) != 0) {
        ...
        switch (event->response_type & ~0x80) {
            ...
            case XCB_KEY_RELEASE: {
                // Key press event
                const xcb_key_press_event_t *kb_event = (xcb_key_press_event_t *)event;
                const b8 pressed = event->response_type == XCB_KEY_PRESS;
                const xcb_keycode_t code = kb_event->detail;
                ...
                const keys key = translate_keycode(key_sym);
            ...
            case XCB_BUTTON_RELEASE: {
                const xcb_button_press_event_t *mouse_event = (xcb_button_press_event_t *)event;
                const b8 pressed = event->response_type == XCB_BUTTON_PRESS;
            ...
            case XCB_MOTION_NOTIFY: {
                // Mouse mouse_event
                const xcb_motion_notify_event_t *move_event = (xcb_motion_notify_event_t *)event;
